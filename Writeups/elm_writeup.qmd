...
This document is for the ELM 2024 paper "".
Here we show the following things:

- Wrangle the raw data, which has been preprocessed from individual CSV files
and filtered according to survey results (the *raw* raw data and the survey
data are not included here).
- Fit (only) the lexical decision models reported in the paper
- Create the RT results plots shown in the paper

I'm restricting what's available in this document to be only what was reported
in the paper.
...
Yes this is unfortunate but I do NOT want to recode all the experiments.
...
The former is useful for anyone, the latter is just reusable functions for
my dissertation.
...
source(here::here("Helpers/brms_helpers.R")) # TODO: delete this
```
...
```{r, message = FALSE}
exp_data <- readRDS("Data/01_Processed/exp_data.rds")
...
posterior_residrt_plot <- 
  sosdiss2::plot_posterior_rt_draws2(condition_difference_draws,
                                     slab_scale = .5,
                                     facet_labels = c(higher="Hear cold, see cool",
                                                      lower="Hear cool, see cold")) + 
  scale_y_continuous(labels = scales::label_percent(), limits = c(-.05, .05))+
  scale_x_discrete(labels = c("H*LL", "LH*LL", "L*HLL", "H*LH", "LH*LH", "L*HLH"))

posterior_residrt_plot

save_plot(posterior_residrt_plot,
          "ldonly_basemodel_epred_speedup2",
          rootdir = "Figures",
          use_subfolders = TRUE,
          width =8.5, height = 4.5, scale = 1.2)
```
